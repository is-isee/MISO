...
public:
  ...
  __device__ inline T &operator[](int idx) { return array[idx]; }

  __device__ inline const T &operator[](int idx) const { return array[idx]; }

  void copy_from_host(const Array3D<T> &array_host) {
    cudaMemcpy(array, array_host.data(), sizeof(T) * i_total * j_total * k_total,
               cudaMemcpyHostToDevice);
  }

  void copy_to_host(Array3D<T> &array_host) const {
    cudaMemcpy(array_host.data(), array, sizeof(T) * i_total * j_total * k_total,
               cudaMemcpyDeviceToHost);
  }

  void copy_from_device(const Array3DDevice<T> &array_device) {
    cudaMemcpy(array, array_device.data(),
               sizeof(T) * i_total * j_total * k_total, cudaMemcpyDeviceToDevice);
  }
