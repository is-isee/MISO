...
namespace artificial_viscosity {
    ...
    __global__ void characteristic_velocity_eval_kernel(
        ...
        Real eos_gm,
        Real cs_fac, Real ca_fac, Real vv_fac) {
    ...
    __global__ void update_ro_kernel(
        ...
        GridDevice<Real> grid,
        Real * dxyzi,
        int i0_, int i1_, int j0_, int j1_, int k0_, int k1_,
        ...
        if (i >= i0_ && i < i1_ && j >= j0_ && j < j1_ && k >= k0_ && k < k1_) {
            ...
            qq_rslt.ro[grid.idx(i, j, k)] = qq.ro[grid.idx(i, j, k)] - (fro_up - fro_dw)*dxyzi[i*is + j*js + k*ks]*dt;
        }
    }
    ...
    __global__ void update_vx_kernel(
        ...
        if (i >= i0_ && i < i1_ && j >= j0_ && j < j1_ && k >= k0_ && k < k1_) {
            ...
            // x momentum
            Real qql2 = qq.ro[grid.idx(i-2*is, j-2*js, k-2*ks)]*qq.vx[grid.idx(i-2*is, j-2*js, k-2*ks)];
            Real qql1 = qq.ro[grid.idx(i-  is, j-  js, k-  ks)]*qq.vx[grid.idx(i-  is, j-  js, k-  ks)];
            Real qqc  = qq.ro[grid.idx(i     , j     , k     )]*qq.vx[grid.idx(i     , j     , k     )];
            Real qqr1 = qq.ro[grid.idx(i+  is, j+  js, k+  ks)]*qq.vx[grid.idx(i+  is, j+  js, k+  ks)];
            Real qqr2 = qq.ro[grid.idx(i+2*is, j+2*js, k+2*ks)]*qq.vx[grid.idx(i+2*is, j+2*js, k+2*ks)];
            // dqq at i-is, j-js, k-2ks
            Real dqq_dw = artificial_viscosity::dqq_eval(qql2, qql1, qqc, ep);
            // dqq at i, j, k
            Real dqq_cn = artificial_viscosity::dqq_eval(qql1, qqc , qqr1, ep);
            ...
            qq_rslt.vx[grid.idx(i, j, k)] = (
                qq.ro[grid.idx(i, j, k)]*qq.vx[grid.idx(i, j, k)] - (frx_up - frx_dw)*dxyzi[i*is + j*js + k*ks]*dt
    ...
    __global__ void update_vy_kernel(
        ...
        if (i >= i0_ && i < i1_ && j >= j0_ && j < j1_ && k >= k0_ && k < k1_) {
            ...
            // x momentum
            Real qql2 = qq.ro[grid.idx(i-2*is, j-2*js, k-2*ks)]*qq.vy[grid.idx(i-2*is, j-2*js, k-2*ks)];
            Real qql1 = qq.ro[grid.idx(i-  is, j-  js, k-  ks)]*qq.vy[grid.idx(i-  is, j-  js, k-  ks)];
            Real qqc  = qq.ro[grid.idx(i     , j     , k     )]*qq.vy[grid.idx(i     , j     , k     )];
            Real qqr1 = qq.ro[grid.idx(i+  is, j+  js, k+  ks)]*qq.vy[grid.idx(i+  is, j+  js, k+  ks)];
            Real qqr2 = qq.ro[grid.idx(i+2*is, j+2*js, k+2*ks)]*qq.vy[grid.idx(i+2*is, j+2*js, k+2*ks)];
            // dqq at i-is, j-js, k-2ks
            Real dqq_dw = artificial_viscosity::dqq_eval(qql2, qql1, qqc, ep);
            // dqq at i, j, k
            Real dqq_cn = artificial_viscosity::dqq_eval(qql1, qqc , qqr1, ep);
            ...
            qq_rslt.vy[grid.idx(i, j, k)] = (
                qq.ro[grid.idx(i, j, k)]*qq.vy[grid.idx(i, j, k)] - (fry_up - fry_dw)*dxyzi[i*is + j*js + k*ks]*dt
            )/qq_rslt.ro[grid.idx(i, j, k)];
        }
    }
    ...
    __global__ void update_vz_kernel(
        ...
        if (i >= i0_ && i < i1_ && j >= j0_ && j < j1_ && k >= k0_ && k < k1_) {
            ...
            // x momentum
            Real qql2 = qq.ro[grid.idx(i-2*is, j-2*js, k-2*ks)]*qq.vz[grid.idx(i-2*is, j-2*js, k-2*ks)];
            Real qql1 = qq.ro[grid.idx(i-  is, j-  js, k-  ks)]*qq.vz[grid.idx(i-  is, j-  js, k-  ks)];
            Real qqc  = qq.ro[grid.idx(i     , j     , k     )]*qq.vz[grid.idx(i     , j     , k     )];
            Real qqr1 = qq.ro[grid.idx(i+  is, j+  js, k+  ks)]*qq.vz[grid.idx(i+  is, j+  js, k+  ks)];
            Real qqr2 = qq.ro[grid.idx(i+2*is, j+2*js, k+2*ks)]*qq.vz[grid.idx(i+2*is, j+2*js, k+2*ks)];
            // dqq at i-is, j-js, k-2ks
            Real dqq_dw = artificial_viscosity::dqq_eval(qql2, qql1, qqc, ep);
            // dqq at i, j, k
            Real dqq_cn = artificial_viscosity::dqq_eval(qql1, qqc , qqr1, ep);
            ...
            qq_rslt.vz[grid.idx(i, j, k)] = (
                qq.ro[grid.idx(i, j, k)]*qq.vz[grid.idx(i, j, k)] - (frz_up - frz_dw)*dxyzi[i*is + j*js + k*ks]*dt
            )/qq_rslt.ro[grid.idx(i, j, k)];
        }
    }
    ...
    __global__ void update_bx_kernel(
        ...
        if (i >= i0_ && i < i1_ && j >= j0_ && j < j1_ && k >= k0_ && k < k1_) {
            ...
            qq_rslt.bx[grid.idx(i, j, k)] = qq.bx[grid.idx(i, j, k)] - (fbx_up - fbx_dw)*dxyzi[i*is + j*js + k*ks]*dt;
    ...
    __global__ void update_by_kernel(
        ...
        if (i >= i0_ && i < i1_ && j >= j0_ && j < j1_ && k >= k0_ && k < k1_) {
            ...
            qq_rslt.by[grid.idx(i, j, k)] = qq.by[grid.idx(i, j, k)] - (fby_up - fby_dw)*dxyzi[i*is + j*js + k*ks]*dt;
    ...
    __global__ void update_bz_kernel(
        ...
        if (i >= i0_ && i < i1_ && j >= j0_ && j < j1_ && k >= k0_ && k < k1_) {
            ...
            qq_rslt.bz[grid.idx(i, j, k)] = qq.bz[grid.idx(i, j, k)] - (fbz_up - fbz_dw)*dxyzi[i*is + j*js + k*ks]*dt;
        }
    }
    ...
    __global__ void update_ph_kernel(
        ...
        if (i >= i0_ && i < i1_ && j >= j0_ && j < j1_ && k >= k0_ && k < k1_) {
            ...
            qq_rslt.ph[grid.idx(i, j, k)] = qq.ph[grid.idx(i, j, k)] - (fph_up - fph_dw)*dxyzi[i*is + j*js + k*ks]*dt;
        }
    }    
    ...
    __global__ void update_ei_kernel(
        ...
        if (i >= i0_ && i < i1_ && j >= j0_ && j < j1_ && k >= k0_ && k < k1_) {
            ...
            // total energy
            Real qql2 = qq.ro[grid.idx(i-2*is, j-2*js, k-2*ks)]*qq.ei[grid.idx(i-2*is, j-2*js, k-2*ks)];
            Real qql1 = qq.ro[grid.idx(i-  is, j-  js, k-  ks)]*qq.ei[grid.idx(i-  is, j-  js, k-  ks)];
            Real qqc  = qq.ro[grid.idx(i     , j     , k     )]*qq.ei[grid.idx(i     , j     , k     )];
            Real qqr1 = qq.ro[grid.idx(i+  is, j+  js, k+  ks)]*qq.ei[grid.idx(i+  is, j+  js, k+  ks)];
            Real qqr2 = qq.ro[grid.idx(i+2*is, j+2*js, k+2*ks)]*qq.ei[grid.idx(i+2*is, j+2*js, k+2*ks)];
            // dqq at i-is, j-js, k-2ks
            Real dqq_dw = artificial_viscosity::dqq_eval(qql2, qql1, qqc, ep);
            // dqq at i, j, k
            Real dqq_cn = artificial_viscosity::dqq_eval(qql1, qqc , qqr1, ep);
...
struct ArtificialViscosity {
    ...
    void characteristic_velocity_eval() {
        artificial_viscosity::characteristic_velocity_eval_kernel<Real><<<cuda.grid_dim, cuda.block_dim>>>(
    ...
    void update(std::vector<Real>& dxyzi, Real* dxyzi_d, std::string direction) {
        ...
        artificial_viscosity::update_ro_kernel<Real><<<cuda.grid_dim, cuda.block_dim>>>(
            mhd_d.qq, mhd_d.qq_rslt, cc_d, grid_d, dxyzi_d, i0_, i1_, j0_, j1_, k0_, k1_,
            is, js, ks, ep, fh, time.dt
        );

        artificial_viscosity::update_vx_kernel<Real><<<cuda.grid_dim, cuda.block_dim>>>(
            mhd_d.qq, mhd_d.qq_rslt, cc_d, grid_d, dxyzi_d, i0_, i1_, j0_, j1_, k0_, k1_,
            is, js, ks, ep, fh, time.dt
        );

        artificial_viscosity::update_vy_kernel<Real><<<cuda.grid_dim, cuda.block_dim>>>(
            mhd_d.qq, mhd_d.qq_rslt, cc_d, grid_d, dxyzi_d, i0_, i1_, j0_, j1_, k0_, k1_,
            is, js, ks, ep, fh, time.dt
        );

        artificial_viscosity::update_vz_kernel<Real><<<cuda.grid_dim, cuda.block_dim>>>(
            mhd_d.qq, mhd_d.qq_rslt, cc_d, grid_d, dxyzi_d, i0_, i1_, j0_, j1_, k0_, k1_,
            is, js, ks, ep, fh, time.dt
        );

        artificial_viscosity::update_bx_kernel<Real><<<cuda.grid_dim, cuda.block_dim>>>(
            mhd_d.qq, mhd_d.qq_rslt, cc_d, grid_d, dxyzi_d, i0_, i1_, j0_, j1_, k0_, k1_,
            is, js, ks, ep, fh, time.dt
        );

        artificial_viscosity::update_by_kernel<Real><<<cuda.grid_dim, cuda.block_dim>>>(
            mhd_d.qq, mhd_d.qq_rslt, cc_d, grid_d, dxyzi_d, i0_, i1_, j0_, j1_, k0_, k1_,
            is, js, ks, ep, fh, time.dt
        );

        artificial_viscosity::update_bz_kernel<Real><<<cuda.grid_dim, cuda.block_dim>>>(
            mhd_d.qq, mhd_d.qq_rslt, cc_d, grid_d, dxyzi_d, i0_, i1_, j0_, j1_, k0_, k1_,
            is, js, ks, ep, fh, time.dt
        );

        artificial_viscosity::update_ph_kernel<Real><<<cuda.grid_dim, cuda.block_dim>>>(
            mhd_d.qq, mhd_d.qq_rslt, cc_d, grid_d, dxyzi_d, i0_, i1_, j0_, j1_, k0_, k1_,
            is, js, ks, ep, fh, time.dt
        );
        
        artificial_viscosity::update_ei_kernel<Real><<<cuda.grid_dim, cuda.block_dim>>>(
            mhd_d.qq, mhd_d.qq_rslt, cc_d, grid_d, dxyzi_d, i0_, i1_, j0_, j1_, k0_, k1_,
            is, js, ks, ep, fh, time.dt
        );
