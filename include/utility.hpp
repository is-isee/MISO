...
namespace util {
    ...
    // for float min
    HOST_DEVICE inline float fmin_safe(float a, float b) {
        return fminf(a, b);
    }

    // for float max
    HOST_DEVICE inline float fmax_safe(float a, float b) {
        return fmaxf(a, b);
    }

    // for double min
    HOST_DEVICE inline double dmin_safe(double a, double b) {
        return fmin(a, b);
    }

    // for double max
    HOST_DEVICE inline double dmax_safe(double a, double b) {
        return fmax(a, b);
    }

    template <typename T>
    HOST_DEVICE inline T max2(T a, T b) {
        return fmax_safe(a, b);
    }

    template <typename T>
    HOST_DEVICE inline T min2(T a, T b) {
        return fmin_safe(a, b);
    }

    template <typename T>
    HOST_DEVICE inline T min3(T a, T b, T c) {
        return fmin_safe(fmin_safe(a, b), c);
    }

    template <typename T>
    HOST_DEVICE inline T max3(T a, T b, T c) {
