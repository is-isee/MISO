...
struct MHD {
    ...
    void save(const Config& config, const Time<Real>& time) const {
        std::string filename = config.mhd_save_dir + 
    ...
    void mpi_exchange_halo(MHDCore<Real>& qq_trgt, Grid<Real>& grid, MPIManager<Real>& mpi) {
        std::array<Array3D<Real>*, 9> vars = {
            &qq_trgt.ro, &qq_trgt.vx, &qq_trgt.vy, &qq_trgt.vz,
            &qq_trgt.bx, &qq_trgt.by, &qq_trgt.bz,
            &qq_trgt.ei, &qq_trgt.ph
        };
            ...
            for (int i = 0; i < grid.i_margin; ++i) {
                for (int j = 0; j < grid.j_total; ++j) {
                    for (int k = 0; k < grid.k_total; ++k) {
                        for (int v = 0; v < 9; ++v) {
                            send_buff_x_plus(i, j, k, v) = (*vars[v])(grid.i_total - 2*grid.i_margin + i, j, k);
                        }
                    }
                }
            }
            MPI_Isend(send_buff_x_plus.data(), send_buff_x_plus.size(), mpi_type<Real>(),
            mpi.x_procs_plus, 100, mpi.cart_comm, &reqs[req_count++]);
            MPI_Irecv(recv_buff_x_plus.data(), recv_buff_x_plus.size(), mpi_type<Real>(),
            mpi.x_procs_plus, 200, mpi.cart_comm, &reqs[req_count++]);
        }
        ...
        if (mpi.x_procs_mnus != MPI_PROC_NULL && grid.i_size > 1) {
            ...
            MPI_Isend(send_buff_x_mnus.data(), send_buff_x_mnus.size(), mpi_type<Real>(),
            mpi.x_procs_mnus, 200, mpi.cart_comm, &reqs[req_count++]);
            MPI_Irecv(recv_buff_x_mnus.data(), recv_buff_x_mnus.size(), mpi_type<Real>(),
            mpi.x_procs_mnus, 100, mpi.cart_comm, &reqs[req_count++]);
        }
        ...
        if (mpi.y_procs_plus != MPI_PROC_NULL && grid.j_size > 1) {
            for (int i = 0; i < grid.i_total; ++i) {
                for (int j = 0; j < grid.j_margin; ++j) {
                    for (int k = 0; k < grid.k_total; ++k) {
                        for (int v = 0; v < 9; ++v) {
                            send_buff_y_plus(i, j, k, v) = (*vars[v])(i, grid.j_total - 2*grid.j_margin + j, k);
                        }
                    }
                }
            }
            MPI_Isend(send_buff_y_plus.data(), send_buff_y_plus.size(), mpi_type<Real>(),
            mpi.y_procs_plus, 300, mpi.cart_comm, &reqs[req_count++]);
            MPI_Irecv(recv_buff_y_plus.data(), recv_buff_y_plus.size(), mpi_type<Real>(),
            mpi.y_procs_plus, 400, mpi.cart_comm, &reqs[req_count++]);
        }
        ...
        if (mpi.y_procs_mnus != MPI_PROC_NULL && grid.j_size > 1) {
            ...
            MPI_Isend(send_buff_y_mnus.data(), send_buff_y_mnus.size(), mpi_type<Real>(),
            mpi.y_procs_mnus, 400, mpi.cart_comm, &reqs[req_count++]);
            MPI_Irecv(recv_buff_y_mnus.data(), recv_buff_y_mnus.size(), mpi_type<Real>(),
            mpi.y_procs_mnus, 300, mpi.cart_comm, &reqs[req_count++]);
        }
            ...
            for (int i = 0; i < grid.i_total; ++i) {
                for (int j = 0; j < grid.j_total; ++j) {
                    for (int k = 0; k < grid.k_margin; ++k) {
                        for (int v = 0; v < 9; ++v) {
                            send_buff_z_plus(i, j, k, v) = (*vars[v])(i, j, grid.k_total - 2*grid.k_margin + k);
                        }
                    }
                }
            }
            MPI_Isend(send_buff_z_plus.data(), send_buff_z_plus.size(), mpi_type<Real>(),
            mpi.z_procs_plus, 500, mpi.cart_comm, &reqs[req_count++]);
            MPI_Irecv(recv_buff_z_plus.data(), recv_buff_z_plus.size(), mpi_type<Real>(),
            mpi.z_procs_plus, 600, mpi.cart_comm, &reqs[req_count++]);
        }
        ...
        if (mpi.z_procs_mnus != MPI_PROC_NULL && grid.k_size > 1) {
            ...
            MPI_Isend(send_buff_z_mnus.data(), send_buff_z_mnus.size(), mpi_type<Real>(),
            mpi.z_procs_mnus, 600, mpi.cart_comm, &reqs[req_count++]);
            MPI_Irecv(recv_buff_z_mnus.data(), recv_buff_z_mnus.size(), mpi_type<Real>(),
            mpi.z_procs_mnus, 500, mpi.cart_comm, &reqs[req_count++]);
        }         
        ...
        if (mpi.x_procs_plus != MPI_PROC_NULL && grid.i_size > 1) {
            for (int v = 0; v < 9; ++v) {
                for (int i = 0; i < grid.i_margin; ++i) {
                    for (int j = 0; j < grid.j_total; ++j) {
                        for (int k = 0; k < grid.k_total; ++k) {
                            (*vars[v])(grid.i_total - grid.i_margin + i, j, k) = recv_buff_x_plus(i, j, k, v);
                        }
                    }
                }
            }
        ...
        if (mpi.y_procs_plus != MPI_PROC_NULL && grid.j_size > 1) {
            for (int v = 0; v < 9; ++v) {
                for (int i = 0; i < grid.i_total; ++i) {
                    for (int j = 0; j < grid.j_margin; ++j) {
                        for (int k = 0; k < grid.k_total; ++k) {
                            (*vars[v])(i, grid.j_total - grid.j_margin + j, k) = recv_buff_y_plus(i, j, k, v);
                        }
                    }
                }
            }
        ...
        if (mpi.z_procs_plus != MPI_PROC_NULL && grid.k_size > 1) {
            for (int v = 0; v < 9; ++v) {
                for (int i = 0; i < grid.i_total; ++i) {
                    for (int j = 0; j < grid.j_total; ++j) {
                        for (int k = 0; k < grid.k_margin; ++k) {
                            (*vars[v])(i, j, grid.k_total - grid.k_margin + k) = recv_buff_z_plus(i, j, k, v);
                        }
                    }
                }
            }
