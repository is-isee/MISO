...
namespace bnd {
    ...
    __global__ void symmetric_kernel(Real* arr, GridDevice<Real> grid, Real sign, Direction direction, Side side) {
        ...
        if (i >= i0_ && i < i1_ && j >= j0_ && j < j1_ && k >= k0_ && k < k1_) {
            ...
            arr[grid.idx(i_ghst, j_ghst, k_ghst)] = sign * arr[grid.idx(i_trgt, j_trgt, k_trgt)];
        }
    }

    template <typename Real>
    __global__ void periodic_kernel(Real* arr, GridDevice<Real> grid, Direction direction, Side side) {
        int i = blockIdx.x * blockDim.x + threadIdx.x;
        int j = blockIdx.y * blockDim.y + threadIdx.y;
        int k = blockIdx.z * blockDim.z + threadIdx.z;
    ...
    template <typename Real>
    void symmetric(Real* arr, const GridDevice<Real>& grid, Real* fac, Real sign, Direction direction, Side side){
        ...
        symmetric_kernel<Real><<<grid_dim, block_dim>>>(arr, grid, sign, direction, side);
        CUDA_CHECK(cudaGetLastError());
        CUDA_CHECK(cudaDeviceSynchronize());
    };

    template <typename Real>
    void periodic(Real* arr, const GridDevice<Real>& grid, Direction direction, Side side) {
