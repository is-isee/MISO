...
struct StandardBoundaryCondition : public BoundaryConditionBase<Real, MHDCoreType, GridType> {
    Config& config;
    GridType& grid;
    EOS<Real>& eos;
    MPIManager<Real>& mpi;
    ...
    void apply(MHDCoreType& qq) override {
        ...
        std::unordered_map< std::string, std::reference_wrapper<Real*> > variables = {
            {"ro", qq.ro}, {"vx", qq.vx}, {"vy", qq.vy}, {"vz", qq.vz},
            {"bx", qq.bx}, {"by", qq.by}, {"bz", qq.bz}, {"ei", qq.ei}, {"ph", qq.ph}
        };
#else
        std::unordered_map<std::string, std::reference_wrapper<Array3D<Real> >> variables = {
            {"ro", qq.ro}, {"vx", qq.vx}, {"vy", qq.vy}, {"vz", qq.vz},
            {"bx", qq.bx}, {"by", qq.by}, {"bz", qq.bz}, {"ei", qq.ei}, {"ph", qq.ph}
        };
#endif
        ...
        constexpr std::array<bnd::Direction, 3> directions = { bnd::Direction::X, bnd::Direction::Y, bnd::Direction::Z };
        constexpr std::array<bnd::Side, 2> sides = { bnd::Side::INNER, bnd::Side::OUTER };
        ...
        for (const auto& [name, array] : variables) {
            for (const auto& direction : directions) {
                ...
                if (is_periodic) {
                    ...
                    conditions = bc_yaml[name][bnd::direction_to_string(direction)].template as<std::vector<std::string>>();
                }
                for (const auto& side : sides) {
                    const std::string method = conditions[static_cast<int>(side)];
                    if (bnd::is_physical_boundary<Real>(direction, side, mpi)) {
                        if (method == "symmetric") {
                            bnd::symmetric<Real>(array.get(), grid, nullptr, 1.0, direction, side);
                        } else if (method == "ro_symmetric") {
#ifdef USE_CUDA                        
                            bnd::symmetric<Real>(array.get(), grid, qq.ro, 1.0, direction, side);
#else
                            bnd::symmetric<Real>(array.get(), grid, &qq.ro, 1.0, direction, side);
#endif
                            ...
                            throw std::runtime_error("Unknown boundary condition method: " + method);
                        }
                    }
                }
            }
