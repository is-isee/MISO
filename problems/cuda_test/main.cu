...
__global__ void initialize_density(MHDDevice<Real> mhd, int i_total, int j_total, int k_total) {
...
int main() {
    ...
    dim3 cuda_grid((model.grid_local.i_total+7)/8, (model.grid_local.j_total+7)/8, (model.grid_local.k_total+7)/8);
    initialize_density<<<cuda_grid, cuda_block>>>(model.mhd_d, model.grid_local.i_total, model.grid_local.j_total, model.grid_local.k_total);
    ...
    for (int i = 0; i < model.grid_local.i_total; ++i) {
        for (int j = 0; j < model.grid_local.j_total; ++j) {
            for (int k = 0; k < model.grid_local.k_total; ++k) {
                std::cout << model.mhd.qq.ro(i, j, k) << " " << i << " " << j << " " << k << std::endl;
            }
        }
    }    
