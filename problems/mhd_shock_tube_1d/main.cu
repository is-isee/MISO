...
template <typename Real>
void initial_condition(Model<Real>& model, const Init<Real>& init, std::string direction);
...
int main() {
    ...
    init.rol = 1.0  ; init.prl = 1.0; init.bpl = +1.0*std::sqrt(4*pi<Real>); init.vvl = 0.0;
    init.ror = 0.125; init.prr = 0.1; init.bpr = -1.0*std::sqrt(4*pi<Real>); init.vvr = 0.0;
    ...
    for(const auto& direction : directions) {
        {
            if(mpi.myrank == 0) {
                std::cout << "####################################################" << std::endl;
                std::cout << "Running simulation in direction: " << direction << std::endl;
                std::cout << "####################################################" << std::endl;
            }
