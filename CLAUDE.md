...
- 物理・数値スキーム・公開 API・依存ライブラリ・リポジトリ構成の変更は、issue で合意する前に実装しない。
...
# フォーマット
find miso demo -name "*.cpp" -o -name "*.hpp" -o -name "*.cu" -o -name "*.cuh" | xargs clang-format --dry-run --Werror -fallback-style=none
