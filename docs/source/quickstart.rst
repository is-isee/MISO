...
To run the GPU version, you need to have NVIDIA HPC SDK installed. If you are running the CPU version, you need to re-run from ``cmake``.

GPU版の実行には、NVIDIA HPC SDKが必要です。CPU版で実行している場合は、 ``cmake`` から再実行する必要があります。
