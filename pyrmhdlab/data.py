...
class Data:
    ...
    def load(self, n_output):
        ...
        for rank in range(self.mpi.n_procs):
            filename = self.mhd_data_dir + 'mhd.' + str(n_output).zfill(self.n_output_digits) + "." + str(rank).zfill(self.n_procs_digits) + '.bin'

            if self.mpi.n_procs > 1:
                ijk_global = [
                    slice(self.mpi.coords['x'][rank]*self.i_size_local,(self.mpi.coords['x'][rank]+1)*self.i_size_local),
                    slice(self.mpi.coords['y'][rank]*self.j_size_local,(self.mpi.coords['y'][rank]+1)*self.j_size_local),
                    slice(self.mpi.coords['z'][rank]*self.k_size_local,(self.mpi.coords['z'][rank]+1)*self.k_size_local)
                ]
            else:
                ijk_global = [
            ...
            with open(filename, 'rb') as f:
                self.ro[tuple(ijk_global)] = np.fromfile(f, dtype=self.endian + self.dtype, count=count).reshape(shape, order='C')[tuple(ijk_local)]
                self.vx[tuple(ijk_global)] = np.fromfile(f, dtype=self.endian + self.dtype, count=count).reshape(shape, order='C')[tuple(ijk_local)]
                self.vy[tuple(ijk_global)] = np.fromfile(f, dtype=self.endian + self.dtype, count=count).reshape(shape, order='C')[tuple(ijk_local)]
                self.vz[tuple(ijk_global)] = np.fromfile(f, dtype=self.endian + self.dtype, count=count).reshape(shape, order='C')[tuple(ijk_local)]
                self.bx[tuple(ijk_global)] = np.fromfile(f, dtype=self.endian + self.dtype, count=count).reshape(shape, order='C')[tuple(ijk_local)]
                self.by[tuple(ijk_global)] = np.fromfile(f, dtype=self.endian + self.dtype, count=count).reshape(shape, order='C')[tuple(ijk_local)]
                self.bz[tuple(ijk_global)] = np.fromfile(f, dtype=self.endian + self.dtype, count=count).reshape(shape, order='C')[tuple(ijk_local)]
                self.ei[tuple(ijk_global)] = np.fromfile(f, dtype=self.endian + self.dtype, count=count).reshape(shape, order='C')[tuple(ijk_local)]
                self.ph[tuple(ijk_global)] = np.fromfile(f, dtype=self.endian + self.dtype, count=count).reshape(shape, order='C')[tuple(ijk_local)]
